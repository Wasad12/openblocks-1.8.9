...
- **Current feature:** Fan — fix loop 1 (1.8.X render path) built, deployed, UNTESTED.
...
- **Last successful build:** `OpenBlocks-1.8.9-1.0.0.jar` (519,022 bytes, reobfuscated, stable_22) — BUILD SUCCESSFUL 2026-09-13 (Fan fix loop 1: 1.8.X Techne/TESR render path, zero compile iterations)
...
- **Next exact action (in order):**
  1. Await user retest of the Fan (checklist in PORTING_LOG.md fix loop 1).
...
- **Last known-good Git commit:** `7871a86` (Fan fix loop 1; matches
  deployed 519,022-byte JAR).
