...
// 1.8.9 port of OpenModsLib ContainerBase: same slot helpers, shift-click transfer
// and custom-slot hooks. 1.8.9 adaptations: nullable stacks (no isEmpty/EMPTY —
// null means empty, stackSize 0 must be normalized to null on put), ICrafting
// instead of IContainerListener (added in 1.9), raw click-type int instead of
// ClickType (added in 1.9). Merge logic null-guarded (the 1.12.2 tryMergeStacks
// assumes non-null stacks).
public abstract class ContainerBase<T> extends Container {
	...
	protected boolean mergeItemStackSafe(ItemStack stackToMerge, int start, int stop, boolean reverse) {
		...
		if (stackToMerge != null && stackToMerge.isStackable()) {
		...
		if (stackToMerge != null && stackToMerge.stackSize > 0) {
			int slotId = reverse? stop - 1 : start;
	...
	public ItemStack transferStackInSlot(EntityPlayer player, int slotId) {
		...
		if (slot != null && canTransferItemOut(slot) && slot.getHasStack()) {
			...
			slot.putStack(itemToTransfer.stackSize > 0? itemToTransfer : null);
	...
	public Set<EntityPlayer> getPlayers() {
		...
		// NOTE: 1.8.9 field is crafters (renamed to listeners in 1.9).
