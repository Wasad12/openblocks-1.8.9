	...
	protected void renderComponentForeground(int x, int y, int mouseX, int mouseY) {
		...
		if (container != null) {
			for (Slot slot : container.inventorySlots) {
				bindComponentsSheet();
				// NOTE: MoreObjects doesn't exist in 1.8.9's guava — plain null check.
				final ISlotBackgroundRenderer renderer = slotRenderers.get(slot.slotNumber);
				(renderer != null? renderer : normalSlot).render(this, slot);
			}
		}
	}
