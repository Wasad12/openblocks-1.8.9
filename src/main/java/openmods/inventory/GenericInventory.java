...
// 1.8.9 port of OpenModsLib GenericInventory: same slot storage/NBT/callback logic.
// 1.8.9 adaptations: nullable ItemStacks in a plain ArrayList (no NonNullList/
// ItemStackHelper in 1.8.9), isUseableByPlayer spelling, IChatComponent display
// name, no isEmpty (added in later versions).
public class GenericInventory implements IInventory {
	...
	protected List<ItemStack> inventoryContents;
	...
	public GenericInventory(String name, boolean isInvNameLocalized, int size) {
		...
		this.inventoryContents = new ArrayList<ItemStack>(size);
	...
	public boolean isItem(int slot, Item item) {
		final ItemStack stack = inventoryContents.get(slot);
	...
	public void clearAndSetSlotCount(int size) {
		this.slotsCount = size;
		inventoryContents = new ArrayList<ItemStack>(size);
		for (int i = 0; i < size; i++)
	...
	public void readFromNBT(NBTTagCompound tag, boolean readSize) {
		...
		inventoryContents = new ArrayList<ItemStack>(this.slotsCount);
		...
		for (int i = 0; i < nbttaglist.tagCount(); i++) {
			...
			if (j >= 0 && j < inventoryContents.size()) {
				final ItemStack stack = ItemStack.loadItemStackFromNBT(stacktag);
				if (stack != null) inventoryContents.set(j, stack);
			}
		}
	}
	...
	public void copyFrom(IInventory inventory) {
		for (int i = 0; i < inventory.getSizeInventory(); i++) {
			if (i < getSizeInventory()) {
				...
				setInventorySlotContents(i, stack != null? stack.copy() : null);
			}
		}
	}
