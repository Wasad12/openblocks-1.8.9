...
// 1.8.9 port of 1.12.2 GuiAutoEnchantmentTable: same tabs/slider/labels/gauge,
// same sync wiring and RPC calls. 1.8.9 adaptations: nullable stacks (no updated
// calls needed here — icons are constructor args), Integer::doubleValue replaced
// with an explicit lambda (same conversion), StandardPalette inlined (tiny const
// holder from the lib — colors only). Everything else verbatim.
public class GuiAutoEnchantmentTable extends GuiConfigurableSlots<TileEntityAutoEnchantmentTable, ContainerAutoEnchantmentTable, TileEntityAutoEnchantmentTable.AutoSlots> {
	...
	protected void addCustomizations(BaseComposite root) {
		...
		addSyncUpdateListener(ValueCopyAction.create(te.getLevelProvider(), slider, value -> value.doubleValue()));
