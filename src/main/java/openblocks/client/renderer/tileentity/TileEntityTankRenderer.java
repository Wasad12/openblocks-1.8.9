...
// Port of 1.12.2 TileEntityTankRenderer: BufferBuilder -> WorldRenderer
// (1.8.9 FastTESR signature); fluid sprite via TextureMapBlocks instead of
// lib TextureUtils. All GL math identical.
public class TileEntityTankRenderer extends FastTESR<TileEntityTank> {

	@Override
	public void renderTileEntityFast(TileEntityTank tankTile, double x, double y, double z, float partialTicks, int destroyStage, WorldRenderer buffer) {
	...
	private static void addVertexWithUV(WorldRenderer wr, double x, double y, double z, double u, double v, int skyLight, int blockLight) {
		wr.pos(x, y, z).color(0xFF, 0xFF, 0xFF, 0xFF).tex(u, v).lightmap(skyLight, blockLight).endVertex();
	}

	private static void renderFluid(final WorldRenderer wr, final ITankRenderFluidData data, float time, int combinedLights) {
		...
		final FluidStack fluidStack = data.getFluid();
