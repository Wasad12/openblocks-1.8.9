...
// Port of 1.12.2 TankRenderLogic. Only change: GenericTank -> FluidTank
// (getSpace() = capacity - amount; the rest of the FluidTank API is identical).
public class TankRenderLogic {
	...
	private static class TankConnections implements ITankConnections {
		...
		public TankConnections(FluidTank tank, Map<Diagonal, DiagonalConnection> diagonalConnections, Map<EnumFacing, HorizontalConnection> horizontalConnections, VerticalConnection topConnection, VerticalConnection bottomConnection) {
			this.tank = tank;
		...
		public void updateFluid(FluidStack fluidStack) {
			...
			topConnection.updateBottomFluid(fluidStack, tank.getCapacity() - tank.getFluidAmount() == 0);
