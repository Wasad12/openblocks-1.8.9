...
public class EntityHangGlider extends Entity implements IEntityAdditionalSpawnData {
	...
	public static boolean isHeldStackDeployedGlider(EntityLivingBase player, ItemStack heldStack) {
		if (player == null || heldStack == null) return false;
	...
	private static boolean isGliderValid(EntityPlayer player, EntityHangGlider glider) {
		...
		if (!isItemHangglider(player.getHeldItem())) return false;
	...
	public void readSpawnData(ByteBuf data) {
		...
		if (e instanceof EntityPlayer) {
			...
			if (OpenBlocks.proxy.isClientPlayer(player))
	...
	public void writeSpawnData(ByteBuf data) {
		if (player == null) {
			logger.warn("Got glider without player id (%s)", this);
			data.writeInt(-42);
		} else {
	...
	protected void entityInit() {
		this.dataWatcher.addObject(PROPERTY_DEPLOYED, (byte)0);
	}
	...
	public double getNoise() {
		double noise = noiseGen.func_151601_a(player.posX / 20f, player.posZ / 20f) / 4d;
		final boolean strong = (noise > 0.7? true : false);
		final int bonus = (strong? THERMAL_STRONG_BONUS_HEIGTH : 0);
		final BlockPos pos = player.getPosition();
		final float biomeRain = worldObj.getBiomeGenForCoords(pos).rainfall;
